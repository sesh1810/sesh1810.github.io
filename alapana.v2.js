...
AM.DRONE=DRONE;
/* ---------------- acoustic violin synth v2 ----------------
   Goal: warm wooden concert-violin character (bright, singing,
   forward mids — tuned toward the South Indian amplified sound).
   Chain: detuned saws + sub  ->  brightness LP (gain-tracked)
        -> modeled body (synthetic impulse-response convolution
           with real violin body modes + formant EQ)
        -> continuous rosin/bow noise tracking the pitch
        -> gentle compression, air shelf, touch of room reverb.
   Pitch curve gets micro "life": slow random-walk jitter (+-4c). */
var VSYNTH=(function(){
  var current=null, bodyIR=null, roomIR=null;

  function makeBodyIR(c){
    // violin body modes (Hz, decay s, amp) — A0 air mode, wood modes, bridge hill
    var modes=[[275,0.09,0.9],[460,0.075,1.0],[560,0.06,0.65],[720,0.05,0.5],
               [980,0.045,0.75],[1300,0.04,0.5],[2100,0.03,0.65],[2900,0.025,0.5],[3600,0.02,0.3]];
    var dur=0.12, n=Math.floor(c.sampleRate*dur);
    var buf=c.createBuffer(1,n,c.sampleRate), d=buf.getChannelData(0);
    for (var i=0;i<n;i++){
      var t=i/c.sampleRate, v=0;
      for (var m=0;m<modes.length;m++){
        v+=modes[m][2]*Math.exp(-t/modes[m][1])*Math.sin(2*Math.PI*modes[m][0]*t);
      }
      v+=0.15*Math.exp(-t/0.01)*(Math.random()*2-1);   // initial knock
      d[i]=v/6;
    }
    return buf;
  }
  function makeRoomIR(c){
    var dur=0.7, n=Math.floor(c.sampleRate*dur);
    var buf=c.createBuffer(2,n,c.sampleRate);
    for (var ch=0; ch<2; ch++){
      var d=buf.getChannelData(ch);
      for (var i=0;i<n;i++){
        var t=i/c.sampleRate;
        d[i]=(Math.random()*2-1)*Math.exp(-t/0.18)*0.5;
      }
    }
    return buf;
  }

  function jitterCurve(n, hopS){
    // slow random walk, clamped to ±4 cents, as a multiplier track
    var out=new Float32Array(n), v=0;
    for (var i=0;i<n;i++){
      v+=(Math.random()-0.5)*0.9*hopS*40;
      v*=0.995;
      if (v>4) v=4; if (v<-4) v=-4;
      out[i]=Math.pow(2, v/1200);
    }
    return out;
  }

  function stop(){
    if (current){ try{ current.forEach(function(nd){ if(nd.stop) nd.stop(); else nd.disconnect(); }); }catch(e){} current=null; }
  }
  ...
  function playContour(contour, octShift, speed, onDone){
    ...
    var jit=jitterCurve(n, hop);
    var freqArr=new Float32Array(n), gainArr=new Float32Array(n), brightArr=new Float32Array(n), noiseF=new Float32Array(n);
    var lastF=220, maxR=0.0001;
    for (var i=0;i<n;i++) maxR=Math.max(maxR, contour.rms[i]);
    for (i=0;i<n;i++){
      var m=contour.midi[i];
      if (!isNaN(m)) lastF=midiToFreq(m+octShift);
      freqArr[i]=lastF*jit[i];
      var g=isNaN(m)?0:Math.min(1, contour.rms[i]/maxR*1.3);
      gainArr[i]=0.0001+0.30*Math.pow(g,0.65);
      brightArr[i]=1400+Math.pow(g,0.8)*4200;
      noiseF[i]=Math.min(8000, Math.max(600, freqArr[i]*2.6));
    }
    for (i=1;i<n;i++){ gainArr[i]=gainArr[i-1]+(gainArr[i]-gainArr[i-1])*0.55; }

    /* ---- nodes ---- */
    var out=c.createGain(); out.gain.value=1;
    var comp=c.createDynamicsCompressor();
    comp.threshold.value=-20; comp.ratio.value=3; comp.attack.value=0.006; comp.release.value=0.2;
    var air=c.createBiquadFilter(); air.type='highshelf'; air.frequency.value=6800; air.gain.value=-5;
    out.connect(air); air.connect(comp); comp.connect(c.destination);
    // room reverb send
    var room=c.createConvolver(); room.buffer=roomIR;
    var roomG=c.createGain(); roomG.gain.value=0.14;
    comp.connect(roomG); roomG.connect(room); room.connect(c.destination);

    // string source
    var master=c.createGain(); master.gain.value=0.0001;
    var o1=c.createOscillator(); o1.type='sawtooth';
    var o2=c.createOscillator(); o2.type='sawtooth'; o2.detune.value=7;
    var o3=c.createOscillator(); o3.type='sawtooth'; o3.detune.value=-6;
    var sub=c.createOscillator(); sub.type='sine';
    var subG=c.createGain(); subG.gain.value=0.16;
    var lp=c.createBiquadFilter(); lp.type='lowpass'; lp.frequency.value=3000; lp.Q.value=0.9;

    // modeled body: convolution with synthetic IR, mixed with dry
    var bodyConv=c.createConvolver(); bodyConv.buffer=bodyIR;
    var wet=c.createGain(); wet.gain.value=0.85;
    var dry=c.createGain(); dry.gain.value=0.4;
    // formant emphasis (the "bridge hill" that gives violin its voice)
    var f1=c.createBiquadFilter(); f1.type='peaking'; f1.frequency.value=980;  f1.gain.value=4;  f1.Q.value=1.2;
    var f2=c.createBiquadFilter(); f2.type='peaking'; f2.frequency.value=2400; f2.gain.value=5;  f2.Q.value=1.6;
    var f3=c.createBiquadFilter(); f3.type='peaking'; f3.frequency.value=450;  f3.gain.value=3;  f3.Q.value=1.1;
    var cut=c.createBiquadFilter(); cut.type='peaking'; cut.frequency.value=1600; cut.gain.value=-2.5; cut.Q.value=2.2;

    o1.connect(lp); o2.connect(lp); o3.connect(lp); sub.connect(subG); subG.connect(lp);
    lp.connect(bodyConv); bodyConv.connect(wet);
    lp.connect(dry);
    var bodySum=c.createGain();
    wet.connect(bodySum); dry.connect(bodySum);
    bodySum.connect(f3); f3.connect(f1); f1.connect(cut); cut.connect(f2); f2.connect(master);
    master.connect(out);

    // continuous rosin noise, bandpass tracking the pitch
    var nb=c.createBufferSource();
    var nlen=Math.floor(c.sampleRate*1.0), nbuf=c.createBuffer(1,nlen,c.sampleRate), nd=nbuf.getChannelData(0);
    for (i=0;i<nlen;i++) nd[i]=Math.random()*2-1;
    nb.buffer=nbuf; nb.loop=true;
    var nf=c.createBiquadFilter(); nf.type='bandpass'; nf.Q.value=1.1;
    var ng=c.createGain(); ng.gain.value=0.0001;
    nb.connect(nf); nf.connect(ng); ng.connect(master);
    var noiseGainArr=new Float32Array(n);
    for (i=0;i<n;i++) noiseGainArr[i]=gainArr[i]*0.11;

    /* ---- schedules ---- */
    var setCurve=function(param, arr){
      try{ param.setValueCurveAtTime(arr, t0, dur); }
      catch(e){
        for (var k=0;k<n;k+=2) param.linearRampToValueAtTime(arr[k], t0+k*hop);
      }
    };
    o1.frequency.setValueAtTime(freqArr[0], t0);
    o2.frequency.setValueAtTime(freqArr[0], t0);
    o3.frequency.setValueAtTime(freqArr[0], t0);
    sub.frequency.setValueAtTime(freqArr[0], t0);
    setCurve(o1.frequency, freqArr);
    setCurve(o2.frequency, freqArr);
    setCurve(o3.frequency, freqArr);
    setCurve(sub.frequency, freqArr);
    setCurve(master.gain, gainArr);
    setCurve(lp.frequency, brightArr);
    setCurve(nf.frequency, noiseF);
    setCurve(ng.gain, noiseGainArr);

    // bow bite at each phrase onset: short bright chirp + noise burst
    contour.segs.forEach(function(seg){
      var ts=t0+seg[0]*hop;
      var bite=c.createOscillator(); bite.type='sawtooth';
      var bf=freqArr[Math.min(n-1,seg[0]+1)];
      bite.frequency.setValueAtTime(bf, ts);
      bite.detune.setValueAtTime(-35, ts);
      bite.detune.linearRampToValueAtTime(0, ts+0.09);
      var bg=c.createGain();
      bg.gain.setValueAtTime(0.10, ts);
      bg.gain.exponentialRampToValueAtTime(0.0001, ts+0.12);
      bite.connect(bg); bg.connect(bodySum);
      bite.start(ts); bite.stop(ts+0.15);
      var nb2=c.createBufferSource();
      var l2=Math.floor(c.sampleRate*0.14), b2=c.createBuffer(1,l2,c.sampleRate), d2=b2.getChannelData(0);
      for (var k2=0;k2<l2;k2++) d2[k2]=Math.random()*2-1;
      nb2.buffer=b2;
      var nf2=c.createBiquadFilter(); nf2.type='bandpass'; nf2.frequency.value=Math.min(7000,bf*3.2); nf2.Q.value=1.4;
      var ng2=c.createGain(); ng2.gain.setValueAtTime(0.10,ts); ng2.gain.exponentialRampToValueAtTime(0.001,ts+0.12);
      nb2.connect(nf2); nf2.connect(ng2); ng2.connect(master);
      nb2.start(ts);
    });

    [o1,o2,o3,sub].forEach(function(o){ o.start(t0); o.stop(t0+dur+0.2); });
    nb.start(t0); nb.stop(t0+dur+0.2);
    current=[o1,o2,o3,sub,nb,out];
    if (onDone) setTimeout(onDone, (dur+0.25)*1000);
    return { t0:t0, dur:dur };
  }
  return { playContour:playContour, stop:stop };
})();
AM.VSYNTH=VSYNTH;
